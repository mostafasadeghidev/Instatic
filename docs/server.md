...
- **When a failure is the author's to fix, carry the reason as DATA and let the handler build the message.** The generic catch is deliberately silent, so a failure that a person could act on has to be caught before it. `PublishRuntimeBuildError` (`server/publish/publishBuildError.ts`) is the worked example: a script importing an undeclared package used to surface as a bare "Internal server error" while the package name, the file and the remedy sat in the server log. It now carries the structured `SiteRuntimeDiagnostic[]`, and `publishBuildErrorMessage` assembles a 400 body **only from diagnostic fields** — never from the thrown `message`, which is exactly what makes it safe to send. Everything else still falls through to the 500.
