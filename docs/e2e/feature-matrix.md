...
| ADMIN-009 | P2 | ✅ | Settings | Close the Settings modal with Escape after focus leaves the dialog | Logged in | Settings modal, click non-focusable chrome, press Escape | The modal closes even though focus sits outside the dialog subtree | dead Escape after blur to body, backdrop-only dismissal |
| ADMIN-010 | P2 | ✅ | Navigation | Reach the AI workspace from the admin section navigation | Owner logged in | Admin section navigation | A gated AI entry renders for capability holders and routes to `/admin/ai` on click | missing nav entry, wrong href, capability leak |
...
CAP-005 note: plugin read/install/configure/lifecycle affordance splits, installed-plugin schedule read vs lifecycle controls, pack re-sync install gating, site-editor AI chat rail gating, and AI provider-manager/auditor tab splits are automated in `capabilities.e2e.ts`. AI write-tool filtering is covered by the unit gate `src/__tests__/agent/aiToolCapabilityGate.test.ts`, not by a browser spec: a non-owner cannot point a provider at a loopback address (GHSA-886f), credentials are per-user, and an owner always holds `ai.tools.write`, so a chat-only persona with a working local provider cannot be built in a browser.
...
| BRANCH-006 | P1 | ✅ | Branches | Write a post on a branch in the Content workspace and merge it | Fresh login (step-up) | Chip → Create branch…; Content → New post → Save draft; review → Merge | The Content workspace follows the tab's branch; the post is absent on main; the review names it by its title; merging with delete lands it on main's Content list | post visible on main before the merge, review naming the row by its id |
...
BRANCH-001 … BRANCH-006 note: `tests/e2e/branches.e2e.ts` drives the real toolbar chip, palette, in-place creator, context strip, manage dialog, preview-link visitor flow (a second browser context without an admin session), the merge review, and a post written in the Content workspace on a branch (absent on main, named by its title in the review, on main after the merge); every step captures evidence under `.tmp/evidence/branches-*.png`. Tests that step up run on a fresh login because step-up rotates the shared owner session.
...
AI-005 note: fixture-backed local Ollama tool-loop coverage is automated in `ai.e2e.ts` for the browser-executed `site_read_document` path: tool request, browser result POST, provider second turn, completed tool badge, and final assistant text. Request-level write-tool filtering for `ai.chat` without `ai.tools.write` is covered by the unit gate `src/__tests__/agent/aiToolCapabilityGate.test.ts`. Mutating write-tool bridge success coverage, timeout/abort UX, duplicate/malformed result handling, permission-denied execution defense, and unknown-tool provider requests remain lower-level or future browser coverage.
...
| CONFIG-004 | P1 | ✅ | Time Zone | See correct timestamps when the server runs outside UTC | CMS process in a non-UTC zone (E2E pins `Europe/Prague`) | Sign in from a second device, Account → Active devices; create two posts, edit the older one | The fresh session reads "Just now", the sessions API returns ISO 8601 UTC timestamps within the last minute, and the edited entry sorts to the top of the Posts list | zone-offset drift on SQLite `current_timestamp` rows, raw `YYYY-MM-DD HH:MM:SS` strings reaching the browser, mixed timestamp shapes in one column |
...
CONFIG-004 note: `tests/e2e/timestamps.e2e.ts` runs with the browser in `America/New_York` against the `Europe/Prague` CMS. It logs the owner in from a second context, asserts `/admin/api/cms/auth/sessions` returns `lastSeenAt` / `createdAt` as ISO 8601 UTC within the last minute, and checks Account → Active devices shows the newest other device as "Just now". Before the SQLite adapter normalised `current_timestamp` rows (PR #527) the stamp read two hours old here; Dashboard Activity was deliberately not used because `audit_events.created_at` already defaults to an ISO `strftime` and never carried the bug. The second test creates two untitled posts, edits the older one, reloads, and expects it first in the Posts list: before writers bound `nowIso()`, the save stamped `updated_at` in SQLite's space form, which sorts below the newer entry's ISO insert default. `src/__tests__/db/sqlite-timestamp-normalization.test.ts` pins the adapter contract and `db-timestamp-writes.test.ts` bans `current_timestamp` in repository writes and keeps DDL defaults on `*_at` columns. Postgres timestamps arrive as `Date` and were never affected.
