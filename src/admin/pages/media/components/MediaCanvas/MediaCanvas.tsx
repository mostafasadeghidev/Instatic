 ...
 * empty / loading / error states — presentation. What a click, a drag, or a
 * right-click SELECTS is `useMediaSelection`; what a drop MOVES is
 * `useMediaDnd`.
 */
import {
  useState,
...
export function MediaCanvas({ workspace, selectionMode = 'standard' }: MediaCanvasProps) {
  ...
  // Selection rules — modifier-aware click, Select All, and which assets a
  // gesture acts on — live in one hook so the click handler, the drag
  // payload, and the context menu cannot drift apart.
  const {
    handleAssetClick,
    ...
    allVisibleSelected,
  } = useMediaSelection({ workspace, canWrite, selectionMode })
